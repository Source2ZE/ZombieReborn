...
function ToggleRepeatKiller()
    if bRepeatKillerToggle == false then
        ...
        DoEntFireByInstanceHandle(hRelay, "RunScriptCode", "Convars:SetInt('mp_respawn_on_death_ct',0)", 0, nil, nil)
    ...
    if bRepeatKillerToggle == true then
        ...
        DoEntFireByInstanceHandle(hRelay, "RunScriptCode", "Convars:SetInt('mp_respawn_on_death_ct',1)", 0, nil, nil)
